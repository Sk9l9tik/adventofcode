...
void part1(){
    ...
    for (int r = 1; r < (int)g.size(); ++r) {
        fill(next.begin(), next.end(), 0);
        for (int c = 0; c < width; ++c) {
            if (!active[c]) continue;

            // char ch = (c < (int)g[r].size()) ? g[r][c] : '.';
            char ch = g[r][c];
            if (ch == '^') {
                ++ans;
                if (c - 1 >= 0) next[c - 1] = 1;
        ...
        bool any = false;
        for (int c = 0; c < width; ++c) if (active[c]) { any = true; break; }
        if (!any) break;
    }
...
void part2(){
    ...
    vvll cache(glen + 1, vll(width, -1));
    std::function<ll(int, int)> solve = [&](int r, int c) -> ll{
        if (r >= isz(g)) return 1;
        if (cache[r][c] != -1) return cache[r][c];

        ll ans{};

        if (r + 1 < isz(g) && g[r + 1][c] == '^'){
            ans = solve(r+1, c - 1) + solve(r + 1, c + 1);
        }
        else{
            ans = solve(r + 1, c);
        }
