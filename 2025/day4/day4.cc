...
void part1(){
    ...
    auto check = [&](int r, int c){
        ...
        for(int dir = 0; dir < 8; ++dir){
            int nr = r + dy8[dir];
            int nc = c + dx8[dir];
            if (nr < 0 || nr >= g.size()) continue;
            if (nc < 0 || nc >= g[0].size()) continue;
...
void part2(){
    ...
    vvi deg(isz(g), vi(isz(g[0]), 0));
    vvb alive(isz(g), vb(isz(g[0]), 0));
    ...
    for (int i=0;i<isz(g);i++)
        for (int j=0;j<isz(g[0]);j++)
    ...
    while(!q.empty()){
        ...
        for(int dir = 0; dir < 8; ++dir){
            ...
            if (nr < 0 || nr >= isz(g)) continue;
            if (nc < 0 || nc >= isz(g[0])) continue;
            if (!alive[nr][nc]) continue;
