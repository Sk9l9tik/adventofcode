...
void part1(){
    ...
    auto check = [&](ll a) -> ll {
        str s = std::to_string(a);
        ll n = s.size();
        if (n % 2 != 0) return 0;

        str s1 = s.substr(0, n / 2);
        str s2 = s.substr(n / 2);
        // std::cout << s << ' ' << s1 << ' ' << s2 << "\n";
    ...
    while(std::getline(ss1, tmp, ',')){
        auto it = tmp.find('-');
        ll fst = stoll(tmp.substr(0, it));
        ll snd = stoll(tmp.substr(it+1));
        // std::cout << fst << ' ' << snd << '\n';
...
void part2(){
    ...
    auto check = [&](ll a) -> ll {
        ...
        for (ll len = 1; len <= n / 2; ++len) {
            if (n % len != 0) continue; // длина должна делиться ровно

            std::string pat = s.substr(0, len);

            // создаем строку = pat повторенная (n/len) раз
            std::string built;
            built.reserve(n);
            for (ll k = 0; k < n / len; ++k) built += pat;
